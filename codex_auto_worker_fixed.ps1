...
function Run([string]$file, [string[]]$arguments, [switch]$AllowFailure) {
    ...
    if ($exit -ne 0 -and -not $AllowFailure) {
        ...
        throw "Command failed with exit code $exit`: $file $($arguments -join ' ')`n$text"
    }
...
function Invoke-CodexPrompt([string]$prompt) {
    $tmp = Join-Path $env:TEMP ("codex_prompt_" + [guid]::NewGuid().ToString() + ".txt")
    Set-Content -Path $tmp -Value $prompt -Encoding UTF8

    try {
        $promptText = Get-Content -Path $tmp -Raw -Encoding UTF8
        $r = Run "codex" @("exec", $promptText)
        return $r.Output
    }
    finally {
...
"@
    ...
    $codexOut = Invoke-CodexPrompt $prompt
...
"@
        ...
        $fixOut = Invoke-CodexPrompt $prompt
